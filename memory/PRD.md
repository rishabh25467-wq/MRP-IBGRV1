...
- P1: SAP MSL Write-Back - user shared a screenshot of the SAP Material Planning tab (Safety Stock=1,000 ea, Procurement Lead Time=4 Days). Confirmed via research both fields ARE writable via OData, but requires a NEW custom OData service in SAP exposing `Material` + `MaterialSupplyPlanningProcessInformation` node (current read-only Communication Arrangements don't expose this node) plus write-authorized business user + CSRF token handling. User said "finish pending tasks, will come back to this" - awaiting their decision on how to proceed (get SAP admin to set up the service vs. read-only first vs. hold off) and scope clarification (MSL->Safety Stock only, or also Procurement Lead Time).
