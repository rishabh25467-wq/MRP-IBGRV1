...
- Tested via testing_agent_v4 (iteration_24): 100% backend (16/16, new `test_category_master.py`) + 100% frontend, both features verified live end-to-end, no regressions on Admin/Purchasing Plan.
- Design nuance noted (not a bug): `save_part_override()` persists the override to Mongo before verifying it resolves, so a failed retry attempt overwrites any previously-working override for that part_no (last-write-wins) - acceptable per current UX, flagged for awareness only.
- Follow-up fix (same session): added ability to DELETE a category from the Manage Categories dialog (X button on each badge) - `DELETE /api/admin/categories/{name}`, `bom_categorizer.delete_category()`. Components already assigned to a deleted category keep their value untouched (just stops appearing in dropdowns/AI taxonomy going forward). Self-tested live in browser (add + delete round-trip verified, toast confirmations correct). Also cleaned up 8 leftover `TEST_*` categories that had accumulated in the shared taxonomy from previous testing_agent_v4 runs.

## Pending (blocked on user)
- P1: SAP MSL Write-Back - user set up a custom OData service (`materialltmsl`, entity `MaterialSupplyPlanningProcessInformation`, Work Center View `PMM_MATERIALS`) exposing `SafetyStockQuantity` (Edm.Decimal, writable) and `PlannedDeliveryDuration` (Edm.String, ISO-8601 duration e.g. "P4D", writable) on Material's Root node, with full CRUD enabled on the entity set. VERIFIED LIVE: queried real data (G12NUT: SafetyStockQuantity=1000, PlannedDeliveryDuration="P4D", SupplyPlanningAreaID="P1") - confirms "P1" is the default/main planning area holding these values (materials have multiple planning-area rows, e.g. P1/P2/P3/P4/P6/P7/P9, but only P1 had non-empty values in both real examples checked). MaterialUUID needed for lookup is already available per-component via existing `product_uuid` field in `bom_node_cache`. CSRF token handling required for the actual PATCH (not yet tested with a real write). Credentials: reuse existing SAP_ODATA_USERNAME/PASSWORD (UNEECOPSTEAM) - need to confirm this business user has write authorization via the new PMM_MATERIALS work center view role (not yet confirmed).
- Awaiting user's answers on scope (asked, not yet answered): (1) always target P1 vs. per-material rule, (2) push MSL only or also Lead Time (source unclear - not tracked in app today), (3) one-way push vs. also pulling SAP's current value into the app for comparison, (4) manual per-row "Push to SAP" button vs. auto-on-save vs. bulk push.
