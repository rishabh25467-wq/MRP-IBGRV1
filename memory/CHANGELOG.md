...
- **Context**: SAP OData action `PGRBackground` (posts Goods Receipt for Inbound Delivery
  Notifications) is genuinely disabled by SAP for this tenant - confirmed via SAP's own official
  KBA 3583076 ("Inability to Post Goods Receipt with Actual Quantities via OData API in Inbound
  Delivery Processing"): Actual Quantity belongs to Confirmed Inbound Delivery, not the Notification,
  and there is no web service/API to create one. Also confirmed live: even with matching/full quantity
  and no override at all, a clean never-touched delivery still gets "action is disabled" via the API.
  Checked and ruled out: Site Logistics Task (`QuerySiteLogisticsTaskIn` - this tenant doesn't use
  task-based execution for inbound, 0 hits even on a wildcard query) and the SAP-official "kh*" sample
  custom OData services from github.com/SAP-samples/byd-api-samples (same ObjectID-only action shape,
  wouldn't help even if deployed - and 3 of 4 aren't even active in this tenant).
- **Also discovered**: PO-sourced (external supplier) stock items don't even get an Inbound Delivery
  Notification created at all in this tenant (searched PO 28792 in the real SAP list - "No records
  found"); every real IDN in this tenant is Stock-Transfer-sourced. That's WHY the Supplier Portal GRN
  flow was using GSA (Goods and Service Acknowledgement) instead - and GSA itself only supports
  non-stock/service PO lines (confirmed live: "PO 28792 item is stock item, cannot use as source
  document for GSA"). Fixed the GSA SOAP namespace bug (`A1S/Global` -> `SAPGlobal20/Global`) in
  `sap_gsa_write_client.py` regardless, since it's still the correct fallback for genuine
  non-stock/service lines.
- **Built** `sap_playwright_pgr_service.py`: headless Playwright automation using the itadmin/
  Admin@1136 UI-capable business user to log into the real SAP ByD UI and click through the actual
  "Post Goods Receipt" screen (Inbound Logistics -> Inbound Delivery Notifications -> search by ID ->
  Post Goods Receipt -> Propose Quantities [or type a specific override into the Actual Quantity grid
  cell] -> Save and Close), verified via re-search (0 hits = gone from the Advised list = posted).
  Runs as a background job (`/api/inbound-receipts/{sto_id}/receive` + `/receive-status/{job_id}`)
  since SAP's own UI takes ~40-90s per delivery - frontend polls and shows a progress bar
  (`InboundReceiptsPage.js`).
- **Bugs found via testing_agent + live testing, all fixed**:
  - Missing `PLAYWRIGHT_BROWSERS_PATH` env var - worked in manual scripts, silently failed on the
    real backend process.
  - "Actual Quantity" never auto-fills from "Planned Quantity" - added "Propose Quantities" click.
  - List-screen toolbar buttons render as `.sapMBtnBase` DIVs, not `<button>` tags (GR review
    sub-screen uses real `<button>`s) - `_click_button` now checks both.
  - Broad `tr:has-text(...)` search-result counting matched the detail panel too (inflated counts,
    never reached 0) - narrowed to `tr[id^="__table"]` (actual list rows only).
  - Error-text extraction ran AFTER navigating away from the failed screen (text already gone) and
    also false-positived on SUCCESS message strips - fixed ordering + now requires "Error" in the text.
  - Re-clicking the "Inbound Logistics" nav icon while already active TOGGLES it closed instead of
    reopening - caused every 2nd+ delivery in a multi-line STO to hard-timeout. Now checks visibility
    of the submenu item first.
  - A failure could leave SAP in an unknown/stuck state - now does a hard reset (fresh page + re-login)
    before the next delivery whenever one fails.
  - "0 hits before even trying" was being reported as `"received"` (false success) - this custom OData
    service has no status field to independently verify against, and it's a live tenant other people
    also use, so renamed to a distinct `"skipped"` status (does NOT count as received in the rollup).
  - Quantity override (`build_line_overrides` + `_set_line_actual_quantity`, replacing the old OData
    PATCH which SAP rejects tenant-wide as read-only) was being applied blindly to every delivery in
    the batch, failing deliveries that don't even carry that product - now scoped to the grid's own
    product IDs first.
  - Frontend: catch-all route added to `App.js`; `overflow-x-auto` on the list table; max-qty/over-
    receipt validation on the Received Qty input; STO-level job lock (`kind: "inbound_receipt"`) so
    double-submits reuse the same job instead of opening 2 browsers; unbounded poll loop given a 6-min
    deadline; low-contrast sub-line text darkened.
- **Live-verified end-to-end multiple times** against the real production SAP tenant this session:
  several real deliveries successfully posted (e.g. P9D1-4178, P9D1-4179, P9D1-4167), a real quantity
  override correctly typed and scoped, and multi-line STOs correctly processing through failures
  without cascading. One known unrelated SAP data gap surfaces correctly as a real, human-readable
  error: product `100002693-B1`/`-A1` has no active valuation at site P2 (needs the user's SAP team to
  fix in Business Configuration, not a code issue). Delivery `P8D1-193` remains genuinely
  corrupted/disabled in SAP's own UI too (from an old Release-before-PGR sequencing mistake in a much
  earlier session) - needs manual SAP Basis intervention, not fixable by this automation.
- **Not yet done**: Supplier Portal GRN (external, stock items) is NOT yet wired to this Playwright
  automation - only internal STO receipts are. Wiring it requires first finding/creating the IDN for a
  given PO (none exist yet for PO-sourced stock items in this tenant) - scoped as a follow-up, not
  started this session per time constraints.
