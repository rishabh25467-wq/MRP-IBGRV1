...
export default function StoreApprovalPage() {
  ...
  // Ticks while the "Refresh Live Stock Now" full SAP inventory pull is
  // in flight (~47s, same underlying job as the Inventory page's Refresh).
  useEffect(() => {
    if (!refreshingStock) {
      setRefreshElapsed(0);
      return;
  ...
  // "Refresh Live Stock Now" (Aug 2026): force a fresh full SAP inventory
  // pull right now instead of waiting up to 30 min for the scheduled
  // refresh, then re-fetch THIS request so its component locations pick
  // up whatever the pull just found. Never blocks/breaks the screen on
  // failure - just toasts and leaves the existing (still cache-backed)
  // data as-is.
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              {(isPending || isReopenable) && (
                <Button
                  ...
                  title="Force a fresh SAP stock pull now instead of waiting for the scheduled 30-min refresh"
                >
