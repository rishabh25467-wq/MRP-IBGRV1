...
async def get_stock_transfer_delivery_note(sto_id: str):
    try:
        return await asyncio.to_thread(stock_transfer_service.get_delivery_note_data, db, erp_portal_client, sto_id, sap_valuation_client, sap_outbound_delivery_client)
...
async def get_stock_transfer_delivery_note_excel(sto_id: str):
    ...
    try:
        data = await asyncio.to_thread(stock_transfer_service.get_delivery_note_data, db, erp_portal_client, sto_id, sap_valuation_client, sap_outbound_delivery_client)
    except stock_transfer_service.StockTransferOrderNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
async def _run_submit_sto_to_sap_job(job_id: str, sto_id: str):
    try:
        result = await asyncio.to_thread(stock_transfer_service.submit_order_to_sap, db, sap_sto_client, sto_id, job_id, sap_valuation_client, sap_goods_movement_client, sap_inventory_client)
        job_store.update_job(db, job_id, {"status": "done", "result": result, "error": None})
